...
> = async (args, api, extraOptions) => {
  ...
  if (result.error?.status === 401) {
    if (!mutex.isLocked()) {
      ...
      try {
        const state = api.getState() as { auth?: { refresh_token?: string | null } };
        const refreshToken = state.auth?.refresh_token;
        if (!refreshToken) {
          const { clearAuth } = await import('@/screens/auth_screens/services/authSlice');
        ...
        if (refreshResult.data) {
          const { setTokens } = await import('@/screens/auth_screens/services/authSlice');
          api.dispatch(setTokens((refreshResult.data as { data: { access_token: string; refresh_token: string } }).data));
